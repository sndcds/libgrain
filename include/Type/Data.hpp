...
namespace Grain {
    ...
    public:
        Data(int64_t capacity = 1024) noexcept {
            m_capacity = std::max<int64_t>(capacity, 1L);
            m_grow_step = 1024;
            if (m_capacity > 0) {
                m_data = static_cast<uint8_t*>(calloc(m_capacity, 1));
                if (!m_data) {
                    m_capacity = 0;
                }
            }
        ...
        uint8_t* checkCapacity(int64_t size) noexcept {
            if (size > m_capacity) {
                ...
                auto new_data = static_cast<uint8_t*>(std::realloc(m_data, new_capacity));
                if (!new_data) {
                    return nullptr;
                }
    ...
    public:
        [[nodiscard]] int32_t lastErr() const { return m_last_err; }
        [[nodiscard]] int64_t base64Size() const { return m_base64_size; }
        [[nodiscard]] int64_t rawDataSize() const { return m_last_err != kErr_None ? -m_last_err : m_base64_size / 4 * 3 - m_padding; }
        [[nodiscard]] static int64_t rawDataMaxSize(int64_t base64_size) { return base64_size / 4 * 3; }

        [[nodiscard]] static uint8_t code2bits(unsigned char code);
        ...
        const bool encodeFinalize();
        [[nodiscard]] const char* codePtr() {
            return static_cast<char*>(m_codes);
        }
