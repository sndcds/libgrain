...
public:
    ...
    T* mutDataPtr() noexcept { return &data_[0][0]; }
    const T* dataPtr() const noexcept { return &data_[0][0]; }
    ...
    void transformVec2(Vec2<T>& v) const noexcept {
        const T* s = dataPtr();
        T x = s[0] * v.x_ + s[1] * v.y_ + s[2];
        T y = s[3] * v.x_ + s[4] * v.y_ + s[5];
        v.x_ = x;
        v.y_ = y;
    }
    ...
    void transformVec3(Vec3<T>& v) const noexcept {
        const T* s = dataPtr();
        T x = s[0] * v.x_ + s[1] * v.y_ + s[2] * v.z_;
        T y = s[3] * v.x_ + s[4] * v.y_ + s[5] * v.z_;
        T z = s[6] * v.x_ + s[7] * v.y_ + s[8] * v.z_;
        v.x_ = x;
        v.y_ = y;
        v.z_ = z;
    }
