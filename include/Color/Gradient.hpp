 ...
 *  if `two_colored_` is true), optional level curve parameters for non-linear
...
protected:
    ...
    //!< Colors at the stop. If `two_colored_` is false, only m_colors[0] is used.
    //!< If `two_colored_` is true, m_colors[0] and m_colors[1] represent the left and right sides.
    ...
    bool m_two_colored = false;     ///< If true, the stop uses two colors with a hard transition at `pos_`.
