...
public:
    explicit SignalBandPassFilter(int32_t sample_rate) noexcept;
    explicit SignalBandPassFilter(
            int32_t sample_rate,
            FilterType filter_type,
            float low_freq,
            float high_freq) noexcept;
    ~SignalBandPassFilter() noexcept override;

    [[nodiscard]] const char *filterTypeName() const noexcept;

    void setFilterType(FilterType filter_type) noexcept;
    void setFreqRange(float low_freq, float high_freq) noexcept override;
