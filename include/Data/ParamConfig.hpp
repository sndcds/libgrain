...
namespace Grain {
    ...
    public:
        ...
        Fix offset_ = 0;               ///< Center value, used for graphical representations, user interface etc
        int32_t precision_ = 0;        ///< Number of fractional digits
        Fix step_ = 1;
        Fix big_step_ = 10;

    public:
        [[nodiscard]] bool isValidUid() const noexcept { return id_ >= 0; }
        ...
        void writeLineToDataFile(File* file, const Fix& value) const {
            if (file) {
                if (value != default_) {
    ...
    public:
        ParamGroup(ParamConfig* data) noexcept {
            ...
            if (data != nullptr) {
                ...
                param_values_ = static_cast<Fix *>(std::malloc(sizeof(Fix) * param_count_));
                resetParams();
            }
        }
        ...
        [[nodiscard]] int32_t length() const noexcept { return param_count_; }

        [[nodiscard]] ParamConfig* ParamConfigAtIndex(int32_t index) const noexcept {
        ...
        void resetParams() const noexcept {
            if (param_values_) {
        ...
        [[nodiscard]] bool isParamAtIndexDefault(int32_t index) const noexcept {
        ...
        [[nodiscard]] int32_t countNonDefault() const noexcept {
            int32_t n = 0;
            for (int32_t i = 0; i < param_count_; i++) {
        ...
        bool setParamByName(const char* name, const char* value) const noexcept {
        ...
        void writeAllParams(File* file) const {
        ...
        [[nodiscard]] bool boolValue(int32_t index) const noexcept {
            return (param_values_ != nullptr && index >= 0 && index < param_count_) ? static_cast<bool>(param_values_[index].asInt32()) : false;
        }

        [[nodiscard]] int32_t intValue(int32_t index) const noexcept {
            return (param_values_ != nullptr && index >= 0 && index < param_count_) ? param_values_[index].asInt32() : 0;
        }

        [[nodiscard]] double realValue(int32_t index) const noexcept {
            return (param_values_ != nullptr && index >= 0 && index < param_count_) ? param_values_[index].asDouble() : 0;
        }

        [[nodiscard]] Fix fixValue(int32_t index) const noexcept {
            return (param_values_ != nullptr && index >= 0 && index < param_count_) ? param_values_[index] : Fix(0);
        }

        [[nodiscard]] bool setParamBool(int32_t index, bool value) const noexcept {
            return setParamInt(index, (int32_t)value);
        }

        [[nodiscard]] bool setParamInt(int32_t index, int32_t value) const noexcept {
        ...
        [[nodiscard]] bool setParamDouble(int32_t index, double value) const noexcept {
            if (param_values_ != nullptr && index >= 0 && index < param_count_) {
                ParamConfig* sud = &param_config_data_[index];
        ...
        [[nodiscard]] bool setParamFix(int32_t index, const Fix& value) const noexcept {
            if (param_values_ != nullptr && index >= 0 && index < param_count_) {
                ParamConfig* sud = &param_config_data_[index];
                Fix min = sud->min_;
                Fix max = sud->max_;
        ...
        static int32_t countNonDefault(const ParamGroup* pg) noexcept {
            return pg ? pg->countNonDefault() : 0;
        }

        static void writeAllParams(const ParamGroup* pg, File* file) {
            if (pg) { pg->writeAllParams(file); }
        }

        static bool boolValue(const ParamGroup* pg, int32_t index) noexcept {
            return pg ? pg->boolValue(index) : false;
        }

        static int32_t intValue(const ParamGroup* pg, int32_t index) noexcept {
            return pg ? pg->intValue(index) : 0;
        }

        static double realValue(const ParamGroup* pg, int32_t index) noexcept {
            return pg ? pg->realValue(index) : 0;
        }

        static Fix fixValue(const ParamGroup* pg, int32_t index) noexcept {
            return pg ? pg->fixValue(index) : 0;
        }


        static bool setParamBool(const ParamGroup* pg, int32_t index, bool value) noexcept {
            return setParamInt(pg, index, (int32_t)value);
        }

        static bool setParamInt(const ParamGroup* pg, int32_t index, int32_t value) noexcept {
            return pg ? pg->setParamInt(index, value) : false;
        }

        static bool setParamDouble(const ParamGroup* pg, int32_t index, double value) noexcept {
            return pg ? pg->setParamDouble(index, value) : false;
        }

        static bool setParamFix(const ParamGroup* pg, int32_t index, const Fix& value) noexcept {
            return pg ? pg->setParamFix(index, value) : false;
        }

        static bool setParamByName(const ParamGroup* pg, const char* name, const char* value) noexcept {
