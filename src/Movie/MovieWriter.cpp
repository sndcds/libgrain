...
namespace Grain {
    ...
    MovieWriter::~MovieWriter() noexcept {
        delete video_frame_buffer_;
    };
            ...
            void* ref) noexcept
    {
        ...
        const bool has_audio = audio_signal != nullptr;

        const int32_t width = config.width;
        const int32_t height = config.height;

        ref_ptr_ = ref;

        delete video_frame_buffer_;
        video_frame_buffer_ = Image::createRGBAFloat(width, height);
        ...
        // Video
        auto av_video_codec_id = Movie::avVideoCodecId(config.video_codec);
        ...
        video_ctx->time_base = AVRational{ 1, config.video_fps };
        video_ctx->framerate = AVRational{ config.video_fps, 1 };
        video_ctx->gop_size = config.video_fps;
        ...
        if (av_video_codec_id == AV_CODEC_ID_H264) {
            ...
            av_dict_set(&opts, "crf", std::to_string(config.video_quality).c_str(), 0); // quality
            video_ctx->pix_fmt = AV_PIX_FMT_YUV420P; // default for H.264
        }
        else if (av_video_codec_id == AV_CODEC_ID_PRORES) {
            av_dict_set(&opts, "profile", std::to_string(config.video_quality).c_str(), 0);
            switch (config.video_quality) {
                case 0: // Proxy
                case 1: // LT
                case 2: // 422
                case 3: // HQ
        ...
        if (config.video_codec == MovieVideoCodec::H264) {
            video_ctx->color_range = AVCOL_RANGE_MPEG; // Limited / TV range
            video_ctx->color_primaries = AVCOL_PRI_BT709;
            video_ctx->color_trc = AVCOL_TRC_BT709;
            video_ctx->colorspace = AVCOL_SPC_BT709;
        }
        ...
        if (has_audio) {
            ...
            audio_ctx->bit_rate = config.audio_bitrate;
        ...
        if (config.video_codec == MovieVideoCodec::H264) {
            // RGB (full range) -> YUV (limited range, BT.709)
            int in_full  = 1; // RGB is full range
            int out_full = 0; // YUV is limited (H.264 expectation)
        ...
        if (intermediate_fmt != AV_PIX_FMT_NONE) {
            ...
            {
                // YUV444P10 -> YUV422P10 (same range & colorspace)
                int in_full  = 0; // YUV is limited
                int out_full = 0;
                const int* coeffs = sws_getCoefficients(SWS_CS_ITU709);
                sws_setColorspaceDetails(
                    sws_444_to_422,
        ...
        // Write Video
        float pixel[4]{};
        ImageAccess fia(video_frame_buffer_, pixel);

        for (int64_t frame_index = 0; frame_index < video_frame_count; frame_index++) {
            frame_callback(this, frame_index);
        ...
        auto flushEncoder = [&](
            ...
            while (true) {
                const int result =
                    avcodec_receive_packet(codec_context, pkt);
        ...
        flushEncoder(video_ctx, video_stream);

        if (has_audio) {
            flushEncoder(audio_ctx, audio_stream);
        }
