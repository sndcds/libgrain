...
namespace Grain {
    ...
    Image* _mac_loadImageFromFile(const String& file_path, Image::PixelType pixel_type) {
        ...
        try {
            ...
            image = new(std::nothrow) Image(src_color_model, (uint32_t)ns_width, (uint32_t)ns_height, pixel_type);
            if (!image) {
    ...
    void _setImageAccessToNSBitmapImageRep(ImageAccess& image_access, NSBitmapImageRep *bitmap_rep) {
        ...
        ia_setup_info.m_width = (uint32_t)[bitmap_rep pixelsWide];
        ia_setup_info.m_height = (uint32_t)[bitmap_rep pixelsHigh];
        ...
        ia_setup_info.m_pixel_data_ptr = (uint8_t*)[bitmap_rep bitmapData];
        ia_setup_info.m_pixel_data_step = (uint32_t)ns_bits_per_pixel / 8;
        ia_setup_info.m_row_data_step = ns_bytes_per_row > 0 ? (uint32_t)ns_bytes_per_row : ia_setup_info.m_pixel_data_step * ia_setup_info.m_width;
